...
function findArrayContainingComment(
	id: string,
...
export function mergeComment(
	...
	if (comment.parentCommentId) {
		findComment(comment.parentCommentId, comments).children.unshift(comment);
	} else {
...
export function updateComment(
	...
	if (container) {
		const originalComment = container.find(
			(comment) => comment.id === updatedComment.id
		);
		container.splice(container.indexOf(originalComment), 1, {
			...originalComment,
			...updatedComment,
