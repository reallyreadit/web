...
> {
	...
	private readonly _openEditComposer = () => {
		if (
			DateTime.fromISO(
				formatIsoDateAsUtc(this.props.comment.dateCreated)
	...
	private readonly _postCommentRevision = (form: CommentRevisionForm) => {
		return this._asyncTracker
			.addPromise(this.props.onPostCommentRevision(form))
			.then((comment) => {
				this.setState({ compositionState: CompositionState.None });
				return comment;
			});
	...
	private readonly _postCommentAddendum = (form: CommentAddendumForm) => {
		return this._asyncTracker
			.addPromise(this.props.onPostCommentAddendum(form))
			.then((comment) => {
				this.setState({ compositionState: CompositionState.None });
				return comment;
			});
	...
	public render() {
		const commentText = !this.props.comment.dateDeleted
			? this.props.comment.text
			: `This comment was deleted on ${DateTime.fromISO(
					formatIsoDateAsUtc(this.props.comment.dateDeleted)
			...
			>
				...
				{this.state.compositionState === CompositionState.Revision ? (
					<CommentRevisionComposer
						...
						initialHeight={
							(this._textDivRef.current &&
								this._textDivRef.current.offsetHeight + 30) ||
							0
						}
					...
					<>
						<div className="text-wrapper" ref={this._textDivRef}>
							<MarkdownContent
								className={classNames('text', {
									deleted: !!this.props.comment.dateDeleted,
								})}
								onNavTo={this._navTo}
								text={commentText}
							/>
						...
						{this.props.comment.addenda.length ? (
							<ol className="addenda">
								{this.props.comment.addenda.map((addendum) => (
									<li className="addendum" key={addendum.dateCreated}>
										<span className="date">
											Update (
											{DateTime.fromISO(
												formatIsoDateAsUtc(addendum.dateCreated)
											).toLocaleString(DateTime.DATE_SHORT)}
											):
										</span>
										<MarkdownContent
				...
				{this.state.compositionState === CompositionState.None &&
				...
				!this.props.comment.dateDeleted ? (
					<div className="actions">
						{this.props.onPostComment ? (
							this.props.user?.name === this.props.comment.userAccount ? (
								<>
									<Link text="Edit" onClick={this._openEditComposer} />
									<Link text="Delete" onClick={this._openDeleteDialog} />
								</>
							) : (
								<Link text="Reply" onClick={this._openReplyComposer} />
							)
