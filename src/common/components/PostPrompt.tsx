		...
		return (
			<ContentBox className="post-prompt_de6v6u">
				{this.props.article.datesPosted.length ? (
					<>
						<p>
							You posted this article on{' '}
							{formatList(this.props.article.datesPosted.map(formatTimestamp))}.
						</p>
						<Link text="Post Again" onClick={this._post} />
					</>
				) : (
					<>
						<p>{this.props.promptMessage}</p>
						<Button intent="success" onClick={this._post} text="Post" />
					</>
				)}
			</ContentBox>
		);
