...
export default class RatingSelector extends React.PureComponent<{
	...
	private readonly _selectRating = (
		event: React.MouseEvent<HTMLButtonElement>
	) => {
		const value = parseInt(event.currentTarget.value);
		if (value !== this.props.value || this.props.allowDeselect) {
			this.props.onChange(value !== this.props.value ? value : null);
		}
	};
	public render() {
		return (
			<div className="rating-selector_epcgq9">
				...
				<div className="rating-bar">
					{this.props.showLabels ? <label>No</label> : null}
					...
					{this.props.showLabels ? <label>Yes</label> : null}
				</div>
			</div>
		);
	}
