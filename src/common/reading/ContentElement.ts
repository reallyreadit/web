...
export default class ContentElement {
	...
	private _setLines(readState: ReadState) {
		...
		for (var i = 0, wordCount = 0; i < lineCount; i++) {
			var lineWordCount =
				minWordsPerLine + (remainder > 0 ? remainder - --remainder : 0);
			this._lines.push(
				new Line(
					this._lineHeight * i,
					this._lineHeight,
	...
	private _getContentRect() {
		...
		return {
			...
			width:
				rect.width - (this._contentOffset.left + this._contentOffset.right),
			height:
				rect.height - (this._contentOffset.top + this._contentOffset.bottom),
		};
	}
