...
function createRadialClipPath(degrees: number) {
	...
	points.push(
		...quadrants.slice(0, quadrantIndex + 1).map((quadrant) => quadrant.point)
	);
...
export default class ProgressIndicator extends React.Component<Props, State> {
	private readonly _handleAnimationEnd = (event: React.AnimationEvent) => {
		...
		} else if (event.animationName === 'progress-indicator_vgryrl-slide-out') {
	...
	public render() {
		return (
			...
			>
				<div className="handle" onClick={this._toggleVisibility}>
					<Icon
						badge={false}
						name={
							this.state.visibility === Visibility.Initial ||
					...
					<div className="progress">
						<div
							className={classNames('fill', {
								complete: this.props.article.isRead,
							})}
							style={{
								clipPath: createRadialClipPath(
									300 * (this.props.article.percentComplete / 100)
								),
							}}
						></div>
					</div>
