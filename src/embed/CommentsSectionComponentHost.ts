...
import EmbedCommentsSection, {
	Props as CommentsSectionProps,
} from './components/EmbedCommentsSection';
...
export default class CommentsSectionComponentHost extends EmbedComponentHost<
	Services,
	State
> {
	protected readonly _component:
		| React.FunctionComponent<CommentsSectionProps>
		| React.ComponentClass<CommentsSectionProps>;
	...
	constructor(params: {
		...
		this._services = {
			...
			onDeleteComment: (form) =>
				params.services.onDeleteComment(form).then((comment) => {
					this.commentUpdated(comment);
					return comment;
				}),
			onPostArticle: (form) =>
				params.services.onPostArticle(form).then((post) => {
					if (post.comment) {
						this.commentPosted(createCommentThread(post));
					}
					return post;
				}),
			onPostComment: (form) =>
				params.services.onPostComment(form).then((res) => {
					this.commentPosted(res.comment);
				}),
			onPostCommentAddendum: (form) =>
				params.services.onPostCommentAddendum(form).then((comment) => {
					this.commentUpdated(comment);
					return comment;
				}),
			onPostCommentRevision: (form) =>
				params.services.onPostCommentRevision(form).then((comment) => {
					this.commentUpdated(comment);
					return comment;
				}),
		};
