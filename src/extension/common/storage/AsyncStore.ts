...
const promisify = (storage: chrome.storage.StorageArea) => chrome.runtime.getManifest().manifest_version > 2 ?
	...
	// MV2 must use callbacks: convert to Promises
	{
		...
		set: (items: { [key: string]: any; }) => new Promise<void>((resolve, reject) => storage.set(items, () => {
			if (chrome.runtime.lastError) {
				reject(chrome.runtime.lastError)
			} else {
				resolve()
		...
		remove: (keys: string | string[]) => new Promise<void>((resolve, reject) => storage.remove(keys, () => {
			if (chrome.runtime.lastError) {
				reject(chrome.runtime.lastError)
			} else {
				resolve()
...
export default abstract class AsyncStore<T> {
	...
	private _storage: {
		...
		{
			read: async () => {
				if (!this._isInitialized) console.warn("AsyncStore read happened before it was fully initialized.")
				return this._fallbackMemoryValue
			},
			write: async (value: T) => {
				if (!this._isInitialized) console.warn("AsyncStore write happened before it was fully initialized.")
				const oldValue = this._fallbackMemoryValue;
				this._fallbackMemoryValue = value;
				// Emalate a ChromeStorageEvent
				this._onStorageChanged({ [this._key]: { oldValue, newValue: value } }, 'local')
			}
		};
	private _isInitialized = false;
	private _key: string;
	private _storageType: ChromeStorageArea;
	private _onStorageChanged = (changes: ChromeStorageEvent, areaName: ChromeStorageArea) => {
		...
		if (changeForKey) {
			...
			this._eventListeners.forEach(listener => listener(oldValue as T, newValue as T))
		}
	};
	...
	constructor(key: string, defaultValue: T, storageType: ChromeStorageArea = 'local') {
		...
		checkStorage(storageType).then(async () => {
			// use storage
