...
export default class Readability {
	...
	private DIV_TO_P_ELEMS = [ "A", "BLOCKQUOTE", "DL", "DIV", "IMG", "OL", "P", "PRE", "TABLE", "UL", "SELECT" ];
	...
	private _parsedPages: { [key: string]: boolean };

	private _pageETags: { [key: string]: number };

	private _curPageNum: number;

	private log: (...args: any[]) => void;
	constructor(uri: UriDescriptor, doc: Document, options?: {
		debug?: boolean,
	...
	**/
	private _postProcessContent(articleContent: Element) {
	 ...
	 */
	private _someNode<T extends Node>(nodeList: NodeListOf<T> | T[], fn: (node: T) => boolean) {
	...
	private _fixRelativeUris(articleContent: Element) {
		...
		this._forEachNode(links, function(this: Readability, link) {
	 ...
	 * returned.
	 */
	private _nextElement(node: Node) {
		var next = node;
		while (next
	...
	private _replaceBrs(elem: Element) {
		this._forEachNode(this._getAllNodesWithTag(elem, ["br"]), function(this: Readability, br) {
			var next = br.nextSibling as Element;
			...
			if (replaced) {
				...
				while (next) {
					...
					p.appendChild(next);
					next = sibling as Element;
				}
			}
		});
	}

	private _setNodeTag<T extends keyof HTMLElementTagNameMap>(node: Node | JSDOMNode, tag: T) {
		this.log("_setNodeTag", node, tag);
		if ((node as JSDOMNode).__JSDOMParser__) {
			(node as JSDOMNode).localName = tag.toLowerCase();
			(node as JSDOMNode).tagName = tag.toUpperCase();
	...
	private _prepArticle(articleContent: Element) {
		...
		this._removeNodes(articleContent.getElementsByTagName('p'), function (this: Readability, paragraph) {
		...
		this._forEachNode(this._getAllNodesWithTag(articleContent, ["br"]), function(this: Readability, br) {
	...
	**/
	 ...
	 */
	private _getNextNode(node: Node | Element, ignoreSelfAndKids?: boolean) {
	 ...
	 */
	private _getNextNodeNoElementProperties(node: Node | Element, ignoreSelfAndKids?: boolean) {
		function nextSiblingEl(n: Node) {
			do {
				n = n.nextSibling;
		...
		// First check for kids if those aren't being ignored
		if (!ignoreSelfAndKids && (node as Element).children[0]) {
			return (node as Element).children[0];
		}
		// Then for siblings...
	...
	private _getNodeAncestors(node: Node, maxDepth?: number) {
		maxDepth = maxDepth || 0;
		var i = 0, ancestors: Node[] = [];
	...
	**/
	private _grabArticle(page?: Element) {
		...
		while (true) {
			...
			var node = this._doc.documentElement as Element;
			...
			**/
			...
			this._forEachNode(elementsToScore, function(this: Readability, elementToScore) {
				if (!elementToScore.parentNode || typeof((elementToScore.parentNode as Element).tagName) === 'undefined')
					return;
				...
				this._forEachNode(ancestors, function(this: Readability, ancestor, level) {
					if (!(ancestor as Element).tagName)
						return;

					if (typeof(ancestor.readability) === 'undefined') {
						this._initializeNode(ancestor as Element);
						candidates.push(ancestor as Element);
					}
			...
			if (topCandidate === null || topCandidate.tagName === "BODY") {
				...
				if (alternativeCandidateAncestors.length >= MINIMUM_TOPCANDIDATES) {
					...
					while ((parentOfTopCandidate as Element).tagName !== "BODY") {
						var listsContainingThisAncestor = 0;
						for (var ancestorIndex = 0; ancestorIndex < alternativeCandidateAncestors.length && listsContainingThisAncestor < MINIMUM_TOPCANDIDATES; ancestorIndex++) {
							listsContainingThisAncestor += Number(alternativeCandidateAncestors[ancestorIndex].includes(parentOfTopCandidate));
						}
						if (listsContainingThisAncestor >= MINIMUM_TOPCANDIDATES) {
							topCandidate = parentOfTopCandidate as Element;
							break;
						}
				...
				parentOfTopCandidate = topCandidate.parentNode;
				while ((parentOfTopCandidate as Element).tagName != "BODY" && (parentOfTopCandidate as Element).children.length == 1) {
					topCandidate = parentOfTopCandidate as Element;
					parentOfTopCandidate = topCandidate.parentNode;
				}
				if (!topCandidate.readability) {
			...
			parentOfTopCandidate = topCandidate.parentNode;
			var siblings = (parentOfTopCandidate as Element).children;
			...
			if (this._getInnerText(articleContent, true).length < 500) {
				...
				this._someNode(ancestors, function(this: Readability, ancestor) {
					if (!(ancestor as Element).tagName)
						return false;
					var articleDir = (ancestor as Element).getAttribute("dir");
					if (articleDir) {
	...
	**/
	private _removeScripts(doc: Document | HTMLElement) {
	...
	**/
	private _hasSinglePInsideElement(element: Element) {
		...
		return !this._someNode(element.childNodes, function(this: Readability, node) {
	 ...
	 */
	private _hasChildBlockElement(element: Node): boolean {
		return this._someNode(element.childNodes, function(this: Readability, node) {
			return this.DIV_TO_P_ELEMS.indexOf((node as Element).tagName) !== -1 ||
	...
	**/
	private _getInnerText(e: Element, normalizeSpaces?: boolean) {
	...
	**/
	private _cleanStyles(e: Element | Document) {
		e = e || this._doc;
		if (!e)
			return;
		...
		if (typeof (e as Element).removeAttribute === 'function' && (e as Element).className !== 'readability-styled')
			(e as Element).removeAttribute('style');

		// Go until there are no more child nodes
		while (cur !== null) {
			if (cur.nodeType === cur.ELEMENT_NODE) {
				// Remove style attribute(s) :
				if ((cur as Element).className !== "readability-styled")
					(cur as Element).removeAttribute("style");

				this._cleanStyles(cur as Element);
			}
	...
	**/
	private _getLinkDensity(element: Element) {
		...
		this._forEachNode(element.getElementsByTagName("a"), function(this: Readability, linkNode) {
	...
	**/
	private _findBaseUrl() {
		var uri = this._uri;
		var noUrlParams = uri.spec.split("?")[0];
	...
	**/
	private _findNextPageLink(elem: Element) {
		var uri = this._uri;
		var possiblePages: {
			[key: string]: {
				score: number,
		...
		for (var i = 0, il = allLinks.length; i < il; i += 1) {
			...
			while (parentNode) {
				var parentNodeClassAndId = (parentNode as Element).className + ' ' + (parentNode as Element).id;
	...
	private _successfulRequest(request: XMLHttpRequest) {
		return (request.status >= 200 && request.status < 300) ||
				request.status === 304 ||
				 (request.status === 0 && request.responseText);
	}

	private _ajax(url: string, options: {
		postBody?: string,
		...
		function respondToReadyState(this: Readability) {
		...
		request.onreadystatechange = respondToReadyState.bind(this);
	...
	private _appendNextPage(nextPageLink: string) {
		var doc = this._doc;
		this._curPageNum += 1;
		...
		(function(this: Readability, pageUrl: string, thisPage: Element) {
			this._ajax(pageUrl, {
				success: function(this: Readability, r) {
					...
					setTimeout((function(this: Readability) {
	...
	**/
	private _getClassWeight(e: Element) {
	 ...
	 **/
	private _clean(e: Element, tag: string) {
		var isEmbed = ["object", "embed", "iframe"].indexOf(tag) !== -1;

		this._removeNodes(e.getElementsByTagName(tag), function(this: Readability, element) {
			// Allow youtube and vimeo videos through as people usually want to see those.
			if (isEmbed) {
				var attributeValues = [].map.call(element.attributes, function(attr: Attr) {
					return attr.value;
				}).join("|");
	 ...
	 * provided one.
	 */
	private _hasAncestorTag(node: Node, tagName: string, maxDepth?: number, filterFn?: (element: Element) => boolean) {
		maxDepth = maxDepth || 3;
		...
		while (node.parentNode) {
			if (maxDepth > 0 && depth > maxDepth)
				return false;
			if ((node.parentNode as Element).tagName === tagName && (!filterFn || filterFn(node.parentNode as Element)))
				return true;
			node = node.parentNode;
			depth++;
		}
	...
	private _markDataTables(root: Element) {
		...
		for (var i = 0; i < tables.length; i++) {
			...
			var descendantExists = function(tag: string) {
	...
	private _cleanConditionally(e: Element, tag: string) {
		...
		this._removeNodes(e.getElementsByTagName(tag), function(this: Readability, node) {
			// First check if we're in a data table, in which case don't remove us.
			var isDataTable = function(t: HTMLTableElement) {
	 ...
	 **/
	private _cleanMatchedNodes(e: Element, regex: RegExp) {
	...
	**/
	private _cleanHeaders(e: Element) {
		for (var headerIndex = 1; headerIndex < 3; headerIndex += 1) {
			this._removeNodes(e.getElementsByTagName('h' + headerIndex), function (this: Readability, header) {
	 ...
	 */
	public isProbablyReaderable(helperIsVisible?: (node: Node) => boolean) {
		...
		if (brNodes.length) {
			var set = new Set();
			[].forEach.call(brNodes, function(node: Node) {
		...
		// this callback:
		return this._someNode(nodes, function(this: Readability, node) {
			if (helperIsVisible && !helperIsVisible(node))
				return false;
			var matchString = (node as Element).className + " " + (node as Element).id;
			...
			if ((node as Element).matches && (node as Element).matches("li p")) {
				return false;
			}
