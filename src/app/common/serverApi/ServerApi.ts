...
export type FetchFunction<TResult> = (
	callback: (value: Fetchable<TResult>) => void
) => Fetchable<TResult>;
export type FetchFunctionWithParams<TParams, TResult> = (
	params: TParams,
	callback: (value: Fetchable<TResult>) => void
) => Fetchable<TResult>;
export default abstract class {
	...
	private createFetchFunction<TResult>(path: string) {
		return (callback: (value: Fetchable<TResult>) => void) =>
			this.get<TResult>({ path }, callback);
	}
	private createFetchFunctionWithParams<TParams, TResult>(path: string) {
		return (params: TParams, callback: (value: Fetchable<TResult>) => void) =>
			this.get<TResult>({ path, data: params }, callback);
	}
	protected abstract get<T = void>(
		request: Request,
		callback: (data: Fetchable<T>) => void
	): Fetchable<T>;
	...
	public readonly changePassword = (
		...
		return this.post({
			path: '/UserAccounts/ChangePassword',
			data: { currentPassword, newPassword },
		});
	};
	public readonly changeEmailAddress = (email: string) => {
		return this.post<UserAccount>({
			path: '/UserAccounts/ChangeEmailAddress',
			data: { email },
		});
	};
	public readonly getUserAccount = (
		callback: (userAccount: Fetchable<UserAccount>) => void
	) => {
		return this.get<UserAccount>(
			{ path: '/UserAccounts/GetUserAccount' },
			callback
		);
	};
	...
	public readonly getBulkMailings = (
	...
	public readonly getEmailSubscriptions = (
		token: string,
		callback: (request: Fetchable<EmailSubscriptionsRequest>) => void
	) => {
		return this.get(
			{ path: '/UserAccounts/EmailSubscriptions', data: { token } },
			callback
		);
	};
	public readonly updateEmailSubscriptions = (
		token: string,
		preference: NotificationPreference
	) => {
		return this.post({
			path: '/UserAccounts/UpdateEmailSubscriptions',
			data: { token, ...preference },
		});
	};
	public readonly getUserAccountStats = (
		callback: (state: Fetchable<UserAccountStats>) => void
	) => {
		return this.get<UserAccountStats>(
			{ path: '/UserAccounts/Stats' },
			callback
		);
	};

	// Analytics
	public readonly getAdminSubscriptionRevenueReport =
		this.createFetchFunctionWithParams<
			DateRangeQuery,
			AdminRevenueReportResponse
		>('/Analytics/RevenueReport');
	public readonly getArticleIssueReportAnalytics =
		this.createFetchFunctionWithParams<
			DateRangeQuery,
			ArticleIssuesReportRow[]
		>('/Analytics/ArticleIssueReports');
	public readonly getAuthorMetadataAssignmentQueue =
		this.createFetchFunction<AuthorMetadataAssignmentQueueResponse>(
			'/Analytics/AuthorMetadataAssignmentQueue'
		);
	public readonly getConversionAnalytics = this.createFetchFunctionWithParams<
		DateRangeQuery,
		ConversionsReportRow[]
	>('/Analytics/Conversions');
	public readonly getDailyTotalAnalytics = this.createFetchFunctionWithParams<
		DateRangeQuery,
		DailyTotalsReportRow[]
	>('/Analytics/DailyTotals');
	public readonly getSignupAnalytics = this.createFetchFunctionWithParams<
		DateRangeQuery,
		SignupsReportRow[]
	>('/Analytics/Signups');
	public readonly getWeeklyUserActivityReport =
		this.createFetchFunctionWithParams<
			DateRangeQuery,
			WeeklyUserActivityReport[]
		>('/Analytics/WeeklyUserActivityReport');
	public readonly logExtensionRemoval = (installationId: string) =>
		this.post({ path: '/Extension/Uninstall', data: { installationId } });
	public readonly logExtensionRemovalFeedback = (data: {
		installationId: string;
		reason: string;
	}) => this.post({ path: '/Extension/UninstallFeedback', data });
	public readonly logNewPlatformNotificationRequest = (
		data: NewPlatformNotificationRequest
	) => this.post({ path: '/Analytics/NewPlatformNotificationRequest', data });

	// Articles
	public readonly assignAuthorToArticle = (request: AuthorAssignmentRequest) =>
		this.post({ path: '/Articles/AuthorAssignment', data: request });
	public readonly getAotdHistory = this.createFetchFunctionWithParams<
		ArticleQuery,
		PageResult<UserArticle>
	>('/Articles/AotdHistory');
	public readonly getArticle = this.createFetchFunctionWithParams<
		{ slug: string },
		UserArticle
	>('/Articles/Details');
	public readonly getArticleSearchOptions =
		this.createFetchFunction<SearchOptions>('/Articles/SearchOptions');
	public readonly getAuthorArticles = this.createFetchFunctionWithParams<
		AuthorArticleQuery,
		PageResult<UserArticle>
	>('/Articles/Author');
	public readonly getCommunityReads = this.createFetchFunctionWithParams<
		CommunityReadsQuery,
		CommunityReads
	>('/Articles/CommunityReads');
	public readonly getPublisherArticles = this.createFetchFunctionWithParams<
		PublisherArticleQuery,
		PageResult<UserArticle>
	>('/Articles/Publisher');
	public readonly getStarredArticles = this.createFetchFunctionWithParams<
		{ pageNumber: number; minLength?: number; maxLength?: number },
		PageResult<UserArticle>
	>('/Articles/ListStarred');
	public readonly getUserArticleHistory = this.createFetchFunctionWithParams<
		{ pageNumber: number; minLength?: number; maxLength?: number },
		PageResult<UserArticle>
	>('/Articles/ListHistory');
	public readonly rateArticle = (id: number, score: number) =>
		this.post<{ article: UserArticle; rating: Rating }>({
			path: '/Articles/Rate',
			data: { articleId: id, score },
		});
	public readonly searchArticles = (query: SearchQuery) =>
		this.post<PageResult<UserArticle>>({
			path: '/Articles/Search',
			data: query,
		});
	public readonly unassignAuthorFromArticle = (
		request: AuthorUnassignmentRequest
	) => this.post({ path: '/Articles/AuthorUnassignment', data: request });

	// Auth
	public readonly authenticateAppleIdCredential = (
		data: AppleIdCredentialAuthForm
	) =>
		this.post<AuthServiceCredentialAuthResponse>({
			path: '/Auth/AppleIos',
			data,
		});
	public readonly authenticateTwitterCredential = (
		data: TwitterCredentialAuthForm
	) =>
		this.post<AuthServiceCredentialAuthResponse>({
			path: '/Auth/TwitterAuthentication',
			data,
		});
	public readonly requestTwitterBrowserAuthRequestToken = (
		data: TwitterBrowserAuthRequestTokenRequest
	) =>
		this.post<TwitterRequestToken>({
			path: '/Auth/TwitterBrowserAuthRequest',
			data,
		});
	public readonly requestTwitterBrowserLinkRequestToken = () =>
		this.post<TwitterRequestToken>({ path: '/Auth/TwitterBrowserLinkRequest' });
	public readonly requestTwitterWebViewRequestToken = () =>
		this.post<TwitterRequestToken>({ path: '/Auth/TwitterWebViewRequest' });
	public readonly linkTwitterAccount = (data: TwitterCredentialLinkForm) =>
		this.post<AuthServiceAccountAssociation>({
			path: '/Auth/TwitterLink',
			data,
		});

	// Authors
	public readonly assignUserAccountToAuthor = (
		data: AuthorUserAccountAssignmentRequest
	) => this.post({ path: '/Authors/UserAccountAssignment', data });
	public readonly getAuthorProfile = this.createFetchFunctionWithParams<
		AuthorProfileRequest,
		AuthorProfile
	>('/Authors/Profile');

	// Extension
	public readonly sendExtensionInstructions = () =>
		this.post({ path: '/Extension/SendInstructions' });

	// Notifications
	public readonly clearAlerts = (data: ClearAlertForm) =>
		this.post({ path: '/Notifications/ClearAlerts', data });

	// Social
	public readonly deleteComment = (data: CommentDeletionForm) =>
		this.post<CommentThread>({ path: '/Social/CommentDeletion', data });
	public readonly followUser = (data: UserNameForm) =>
		this.post({ path: '/Social/Follow', data });
	public readonly getComments = this.createFetchFunctionWithParams<
		CommentsQuery,
		CommentThread[]
	>('/Social/Comments');
	public readonly getFollowees =
		this.createFetchFunction<Following[]>('/Social/Followees');
	public readonly getFollowers = this.createFetchFunctionWithParams<
		UserNameQuery,
		Following[]
	>('/Social/Followers');
	public readonly getNotificationPosts = this.createFetchFunctionWithParams<
		NotificationPostsQuery,
		PageResult<Post>
	>('/Social/NotificationPosts');
	public readonly getReplyPosts = this.createFetchFunctionWithParams<
		ReplyPostsQuery,
		PageResult<Post>
	>('/Social/ReplyPosts');
	public readonly postArticle = (data: PostForm) =>
		this.post<Post>({ path: '/Social/Post', data });
	public readonly postComment = (data: CommentForm) =>
		this.post<CommentCreationResponse>({ path: '/Social/Comment', data });
	public readonly postCommentAddendum = (data: CommentAddendumForm) =>
		this.post<CommentThread>({ path: '/Social/CommentAddendum', data });
	public readonly postCommentRevision = (data: CommentRevisionForm) =>
		this.post<CommentThread>({ path: '/Social/CommentRevision', data });
	public readonly getPostsFromUser = this.createFetchFunctionWithParams<
		UserPostsQuery,
		PageResult<Post>
	>('/Social/UserPosts');
	public readonly getProfile = this.createFetchFunctionWithParams<
		UserNameQuery,
		Profile
	>('/Social/Profile');
	public readonly unfollowUser = (data: UserNameForm) =>
	...
	public readonly getReadingTimeStats = this.createFetchFunctionWithParams<
		...
		ReadingTimeStats
	>('/Stats/ReadingTime');
	public readonly getLeaderboards = this.createFetchFunction<Leaderboards>(
		'/Stats/Leaderboards'
	);
	public readonly getUserCount = this.createFetchFunction<{
		userCount: number;
	}>('/Stats/UserCount');
	...
	public readonly changeDisplayPreference = (data: DisplayPreference) =>
		this.post<DisplayPreference>({
			path: '/UserAccounts/DisplayPreference',
			data,
		});
	public readonly changeNotificationPreference = (
		data: NotificationPreference
	) =>
		this.post<NotificationPreference>({
			path: '/UserAccounts/NotificationPreference',
			data,
		});
	public readonly changeTimeZone = (timeZone: { id?: number; name?: string }) =>
		this.post<UserAccount>({
			path: '/UserAccounts/ChangeTimeZone',
			data: timeZone,
		});
	public readonly createAuthServiceAccount = (data: AuthServiceAccountForm) =>
		this.post<WebAppUserProfile>({
			path: '/UserAccounts/AuthServiceAccount',
			data,
		});
	public readonly createUserAccount = (data: UserAccountForm) =>
		this.post<WebAppUserProfile>({ path: '/UserAccounts/CreateAccount', data });
	public readonly deleteUserAccount = (request: SignOutForm) =>
		this.post({ path: '/UserAccounts/Deletion', data: request });
	public readonly getDisplayPreference =
		this.createFetchFunction<DisplayPreference>(
			'/UserAccounts/DisplayPreference'
		);
	public readonly getSettings = this.createFetchFunction<Settings>(
		'/UserAccounts/Settings'
	);
	public readonly getTimeZones = this.createFetchFunction<
		TimeZoneSelectListItem[]
	>('/UserAccounts/TimeZones');
	public readonly registerOrientationCompletion = () =>
		this.post<UserAccount>({ path: '/UserAccounts/OrientationCompletion' });
	public readonly requestPasswordReset = (data: PasswordResetRequestForm) =>
		this.post({ path: '/UserAccounts/RequestPasswordReset', data });
	public readonly resetPassword = (data: PasswordResetForm) =>
		this.post<WebAppUserProfile>({ path: '/UserAccounts/ResetPassword', data });
	public readonly sendPasswordCreationEmail = () =>
		this.post({ path: '/UserAccounts/PasswordCreationEmailDispatch' });
	public readonly signIn = (data: SignInForm) =>
		this.post<WebAppUserProfile>({ path: '/UserAccounts/SignIn', data });
	public readonly submitAuthorEmailVerificationRequest = (
		request: AuthorEmailVerificationRequest
	) =>
		this.post({
			path: '/UserAccounts/AuthorEmailVerificationRequest',
			data: request,
		});
}
