...
import {
	...
	RevenueReportLineItem,
} from '../../../../common/models/analytics/RevenueReport';
...
function createPeriodGroups(data: RevenueReportResponse) {
	return data.lineItems
		.reduce<LineItemPeriodGroup[]>((groups, item) => {
			const group = groups.find((group) => group.period === item.period);
			if (group) {
		...
		.sort((a, b) => (b.period < a.period ? -1 : b.period > a.period ? 1 : 0));
}
function getCustomItems(items: RevenueReportLineItem[]) {
	return items.filter((item) => item.priceAmount > 0 && !item.priceName);
}
function renderBody(data: RevenueReportResponse) {
	...
	return (
		<tbody>
			{createPeriodGroups(data).map((group) => {
				const groupTotal = sumNetAmount(group.items),
					customTotal = sumNetAmount(getCustomItems(group.items)),
				...
				return (
					<tr key={group.period}>
						<td className="align-center">
							{group.period.replace(/T00:00:00$/, '')}
						</td>
						...
						<td
							className={classNames('align-right', { null: groupTotal === 0 })}
						>
...
function renderChart(data: RevenueReportResponse) {
	...
	const chartPoints = createPeriodGroups(data)
		.reverse()
		.map((group) => ({
			xLabel: DateTime.fromISO(group.period).toFormat('L/d'),
			...
			monthlyRecurringRevenue: data.monthlyRecurringRevenueLineItems.find(
				(item) => item.period === group.period
			).amount,
		}));
	return (
		<>
			...
			<ChartKey item1="Apple" item2="Stripe" />
		</>
	);
}
function renderProviderPriceCells(
	items: RevenueReportLineItem[],
	price: Price,
	format: 'normal' | 'total' = 'normal'
) {
	const priceItems = items.filter((item) => item.priceName === price),
		appleAmount = sumNetAmount(
			priceItems.filter((item) => item.provider === SubscriptionProvider.Apple)
		),
		stripeAmount = sumNetAmount(
			priceItems.filter((item) => item.provider === SubscriptionProvider.Stripe)
		);
	return (
...
function renderFooter(data: RevenueReportResponse) {
	...
	const customTotal = sumNetAmount(getCustomItems(data.lineItems)),
...
function sumNetAmount(items: RevenueReportLineItem[]) {
	return items.reduce(
		(sum, item) =>
			sum +
			item.quantityPurchased * item.priceAmount -
			item.quantityRefunded * item.priceAmount,
		0
	);
}
