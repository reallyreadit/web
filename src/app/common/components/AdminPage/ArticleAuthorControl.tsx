...
export class ArticleAuthorControl extends React.Component<Props, State> {
	...
	private readonly _changeAction = (
	...
	private readonly _changeArticleReference = (
		event: React.ChangeEvent<HTMLInputElement>
	) => {
		this.setState({
	...
	private readonly _changeAuthorReference = (
		event: React.ChangeEvent<HTMLInputElement>
	) => {
		this.setState({
	...
	private readonly _submit = (event: React.MouseEvent<HTMLButtonElement>) => {
		...
		this.setState((currentState) => {
			...
			if (/^https?:/.test(articleSlug)) {
				try {
					const articleUrl = new URL(articleSlug),
						params = findRouteByKey(routes, ScreenKey.Read).getPathParams(
							articleUrl.pathname
						);
	...
	public render() {
		let authorReferenceLabelText: string, authorReferenceExampleText: string;
		...
		return (
			<form className="article-author-control_r2vdzx">
				<fieldset>
					...
					<label>
						Example: the-new-yorker_the-dead-zone OR
						https://readup.org/read/the-new-yorker/the-dead-zone
					</label>
					<label>
