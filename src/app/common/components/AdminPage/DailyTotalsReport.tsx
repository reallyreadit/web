...
function getHeaders(data?: DailyTotalsReportRow[]) {
	...
	addHeader(
		headers,
		'Signups',
		data?.some((day) => day.signupUnknownCount)
	);
	addHeader(
		headers,
		'Reads',
		data?.some((day) => day.readUnknownCount)
	);
	addHeader(
		headers,
		'Posts',
		data?.some((day) => day.postUnknownCount)
	);
	addHeader(
		headers,
		'Replies',
		data?.some((day) => day.replyUnknownCount)
	);
	headers[0].push(
		{
			name: 'Post Tweets',
...
function renderCell(value: number) {
	return (
		<td className={classNames('align-center', { null: value === 0 })}>
			{value}
		</td>
	);
}
