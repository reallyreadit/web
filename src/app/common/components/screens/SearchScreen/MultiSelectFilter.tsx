...
export default class MultiSelectFilter extends React.Component<Props, State> {
	...
	private readonly _toggleOption = (
		event: React.MouseEvent<HTMLButtonElement>
	) => {
		const slug = event.currentTarget.value,
			nextValue = this.props.value.slice();
		if (this.props.value.includes(slug)) {
		...
		return (
			<Filter
				...
				subtitle={this.props.value
					.map(
						(slug) =>
							this.props.options.find((option) => option.slug === slug).name
					)
					.sort()
					.join(', ')}
				title={this.props.title}
			>
				{this.props.options.slice(0, this.state.displayCount).map((option) => (
					<button
				...
				{this.state.displayCount < this._maxDisplayCount ? (
					<button className="option show-more" onClick={this._showMore}>
						Show More +
					</button>
				) : null}
			</Filter>
