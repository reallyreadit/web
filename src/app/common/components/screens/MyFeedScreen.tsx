...
import AbstractFollowable, {
	noop,
	reloadProfile,
	updateProfile,
} from '../AbstractFollowable';
...
class MyFeedScreen extends AbstractFollowable<Props, State> {
	...
	private clearAlertIfNeeded() {
		if (
			!this._hasClearedAlert &&
			hasAnyAlerts(this.props.userAccount, postAlerts)
		) {
	...
	public componentDidUpdate(prevProps: Props) {
		...
		if (this.props.userAccount && prevProps.userAccount) {
			const newItemCount = Math.max(
				0,
				this.props.userAccount.postAlertCount -
					prevProps.userAccount.postAlertCount +
					(this.props.userAccount.loopbackAlertCount -
						prevProps.userAccount.loopbackAlertCount)
			);
			if (newItemCount) {
				this.setState({
					newItemCount,
