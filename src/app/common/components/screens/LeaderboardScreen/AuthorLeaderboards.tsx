...
export default class AuthorLeaderboards extends React.Component<Props> {
	...
	private renderMobileCard(item: AuthorEarningsReport) {
		...
		return (
			<ContentBox key={item.authorSlug}>
				<div className="author-section">
					<div className="top-line">
						{this.renderAuthorLink(item)}
						<span className="earnings">
							Earned {formatCurrency(item.amountEarned)}{' '}
				...
				<div className="article-section">
					...
					>
						<div className="top-article">
							Top Article <Icon name="arrow-ne" title="Open link to comments" />
						</div>
	...
	private renderDetailsLine(topArticle: UserArticle): React.ReactNode {
		return (
			<span className="details">
				<span className="source" title={topArticle.source}>
					{topArticle.source}
				</span>
				<i className="spacer"></i>
				{calculateEstimatedReadTime(topArticle.wordCount)} min.
				<i className="spacer"></i>
				{topArticle.readCount} reads
			</span>
		);
	}
		...
		return !this.props.response.isLoading &&
			...
			<div className="load-more-button">
				...
				<Button
					...
					size="normal"
					intent="normal"
					onClick={this.props.onLoadMoreAuthors}
					state={
						!!this.props.responseMore && !this.props.responseMore.isLoading
							? 'disabled'
							: !!this.props.responseMore && this.props.responseMore.isLoading
							? 'busy'
							: 'normal'
	...
	private renderDesktopLineItem(
		columns: ColumnDefinition,
		item: AuthorEarningsReport
	): React.ReactNode {
		const [sourceSlug, articleSlug] = item.topArticle.slug.split('_'),
			articleUrlParams = {
				['articleSlug']: articleSlug,
		...
		return (
			<tr key={item.authorSlug}>
				...
				>
					<span className="content">{formatCurrency(item.amountEarned)}</span>
				</td>
				<td
				...
				<table>
					<thead>
						<tr>
							{Object.keys(columns).map((key) => {
								...
								return (
									<th className={column.class} key={key}>
										{column.header}
										{column.class === 'status' && (
											<Icon
					...
					<tbody>
						{this.props.response.value.lineItems.length ? (
							this.props.response.value.lineItems
								.sort((a, b) => b.amountEarned - a.amountEarned)
								.map((item) => this.renderDesktopLineItem(columns, item))
						) : (
							<tr>
								<td colSpan={4}>No earnings found.</td>
							</tr>
						)}
						...
						this.props.responseMore.value.lineItems.length ? (
						  ...
						  this.props.responseMore.value.lineItems.length === 0 ? (
							<tr>
								<td colSpan={4}>No more earnings found.</td>
							</tr>
						) : null}
					</tbody>
				</table>
	...
	public render() {
		return this.props.response.isLoading ? (
			...
			<div
				className={classNames('author-leaderboards_4rtwc1', {
					'logged-in': !!this.props.user,
				})}
			>
				<div className="mobile-container">{this.renderMobileCards()}</div>
				<div className="desktop-container">
					<ContentBox>{this.renderDesktopTable()}</ContentBox>
				</div>
			</div>
		) : (
			<ContentBox>
