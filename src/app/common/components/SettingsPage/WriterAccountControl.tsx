...
interface Props {
	...
	onOpenDialog: (
		renderer: (sharedState: SharedState) => React.ReactNode
	) => void;
...
export class WriterAccountControl extends React.Component<Props> {
	...
	private readonly _openTwitterVerificationDialog = () => {
		this.props.onOpenDialog((sharedState) => (
		...
		return (
			<div className="writer-account-control_yerlvh">
				{this.props.authorProfile ? (
					<ContentBox>
						<p>
							<strong>{this.props.authorProfile.name}</strong>
						</p>
					...
					<>
						<p>
							Are you a writer? Get verified so that you can comment on your own
							articles without having to read them.
						</p>
						<Button
							intent="loud"
