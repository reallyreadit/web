...
export class PostList extends React.Component<Props> {
	...
	constructor(props: Props) {
		...
		this._asyncTracker.addCancellationDelegate(
			...
			props.onRegisterArticlePostedHandler((post) => {
						...
						value: {
							...
							items: [
								...
								...this.props.posts.value.items.slice(
									...
									this.props.posts.value.pageSize
										? this.props.posts.value.items.length
			...
			props.onRegisterCommentUpdatedHandler((comment) => {
				if (this.props.posts.value) {
					const post = this.props.posts.value.items.find(
						(post) => post.comment && post.comment.id === comment.id
					);
					if (post) {
						const items = this.props.posts.value.items.slice();
						if (comment.dateDeleted) {
							items.splice(items.indexOf(post), 1);
						} else {
							items.splice(items.indexOf(post), 1, {
								...post,
								comment: {
	...
	public render() {
		...
		return (
			<div className="post-list_eij9bc">
				<List>
					{this.props.posts.value.items.map((post) => (
						<li key={post.date}>
							<PostDetails
