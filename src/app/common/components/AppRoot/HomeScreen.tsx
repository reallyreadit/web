...
class HomeScreen extends React.Component<Props, State> {
	...
	private readonly _loadMore = () => {
		...
		return new Promise<void>((resolve) => {
			...
			this.props.onGetAotdHistory(
				...
				this._asyncTracker.addCallback((aotdHistory) => {
					resolve();
					this.setState(
						produce((state: State) => {
		...
		this._asyncTracker.addCancellationDelegate(
			props.onRegisterArticleChangeHandler((event) => {
				...
				if (
					...
					this.state.aotdHistory.value.items.some(
						(article) => article.id === event.article.id
					)
				) {
					this.setState(
						produce((prevState: State) => {
							prevState.aotdHistory.value.items.forEach(
	...
	private fetchNewAOTD(
		minLength: number | null,
		maxLength: number | null,
		pageNumber: number,
		sort: Sort
	) {
		const communityPromise = new Promise<Fetchable<CommunityReads>>(
			(resolve, reject) => {
				try {
		...
		this._asyncTracker.addPromise(communityPromise).then((communityReads) => {
			this.setState(
				produce((prevState: State) => {
					...
					if (
						!prevState.aotdHistory.isLoading &&
						this.state.aotdHistory.value
					) {
	...
	private isLoading() {
		return (
			this.state.communityReads.isLoading || this.state.aotdHistory.isLoading
		);
	}
}
