		...
		this._asyncTracker.addCancellationDelegate(
			props.onRegisterArticleChangeHandler((event) => {
				if (
					...
					this.state.article.value.id === event.article.id
				) {
			...
			props.onRegisterCommentPostedHandler((comment) => {
				if (
					this.state.article.value &&
					this.state.article.value.id === comment.articleId &&
...
type Dependencies = Pick<
	...
	Exclude<
		keyof Props,
		'article' | 'articleSlug' | 'highlightedCommentId' | 'location' | 'user'
	>
>;
