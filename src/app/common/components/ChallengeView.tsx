...
> {
	...
	private readonly _reset = () => {
		if (this.state.userPrompt != null) {
			this.setState({ userPrompt: null });
		} else {
			this.forceUpdate();
		}
	};
	...
	private readonly _goToEnrollScreen = () => {
		this.setState({ userPrompt: UserPrompt.Enrolling });
	};
	...
	public componentDidMount() {
		this.context.user
			.addListener('authChange', this._reset)
	...
	public componentWillUnmount() {
		this.context.user
			.removeListener('authChange', this._reset)
	...
	public render() {
		if (
			...
			let
				menuScreen: React.ReactNode | undefined,
				timeZoneName: string | undefined;
			if (!this.context.challenge.latestResponse) {
				if (this.state.userPrompt === UserPrompt.Enrolling) {
					menuScreen = (
						<EnrollScreen
