...
export default class extends ExtensionApi {
	...
	constructor(params?: Params) {
		...
		this._contentScriptMessagingContext.addListener((message, sendResponse) => {
			switch (message.type) {
				case 'articlePosted':
				...
				case 'initialize':
					...
					if (!this.isInstalled) {
						...
						this.changeInstallationStatus(
							new SemanticVersion(message.data?.version || '3.3.0')
						);
					}
					// flush queue
	...
	private changeInstallationStatus(installedVersion: SemanticVersion | null) {
		...
		if (this._extensionVersionCookieAttributes) {
			...
			if (installedVersion && !extensionVersionCookie) {
				Cookies.set(extensionVersionCookieKey, installedVersion.toString(), {
